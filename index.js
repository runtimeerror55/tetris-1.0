...
//       "mongodb+srv://dbUser:dPPEbgZLe0oPy8Oh@atlascluster.swmtc.mongodb.net/?retryWrites=true&w=majority";
// mongoose
...
io.on("connection", (socket) => {
      ...
      socket.on("join room", (roomId, userProfileInformation) => {
            if (connectedRooms.hasOwnProperty(roomId)) {
                  if (
                        ...
                        connectedRooms[roomId]["matchTypeValue"]
                  ) {
                        connectedRooms[roomId]["connectedPlayersDetails"][
                              socket.id
                        ] = {
                              googleId: userProfileInformation["googleId"],
                              name: userProfileInformation["name"],
      ...
      socket.on(
            "connect player",
            (roomId, playerIndex, matchTypeValue, userProfileInformation) => {
                  id = roomId;
      ...
      socket.on("disconnect player number", (roomId, playerIndex) => {
            ...
            socket.broadcast
                  .in(id)
                  .emit("disconnected player number", playerIndex, false);
            console.log("hi");
      });
      ...
      socket.on("disconnecting", () => {
            ...
            if (roomId != undefined) {
                  ...
                  delete connectedRooms[roomId]["connectedPlayersDetails"][
                        socket.id
                  ];
      ...
      socket.on(
            "toggleClass",
            (roomId, coordinates, hover, playerNumber, blockColor) => {
                  socket.broadcast
                        .in(id)
                        .emit(
                              "toggleClass",
      ...
      socket.on(
            "addClass",
            (roomId, previous, currentDpTrueCoordinates, playerNumber) => {
                  socket.broadcast
                        .in(id)
                        .emit(
                              "addClass",
                              previous,
                              currentDpTrueCoordinates,
                              playerNumber
                        );
            }
      );
      ...
      socket.on("laserBeamRow", (roomId, playerNumber, row, width) => {
            socket.broadcast
                  .in(id)
                  .emit("laserBeamRow", playerNumber, row, width);
      });
      ...
      socket.on(
            ...
            (
                  roomId,
                  fullRows,
                  arrayOftotalNoOfBlocksInEachRow,
                  playerNumber,
                  flag
            ) => {
                  socket.broadcast
      ...
      socket.on("anyone connected", (roomId) => {
            if (connectedRooms[roomId]["connectedPlayers"] > 0) {
                  ...
                  if (connectedRooms[roomId] != undefined) {
                        ...
                        playerIndexes = Object.values(
                              connectedRooms[roomId]["connectedPlayersDetails"]
                        ).map((element) => element["playerNumber"]);
                  }
      ...
      socket.on("update score", (roomId, playerIndexValue, score) => {
            socket.broadcast
                  .in(roomId)
                  .emit("update score", playerIndexValue, score);
      });

      socket.on(
            "generatedBlockProperties",
            (roomId, arrayOfObjectsOfBlockProperties) => {
                  socket.broadcast
                        .in(id)
                        .emit(
                              "generatedBlockProperties",
                              arrayOfObjectsOfBlockProperties
                        );
            }
      );
      ...
      socket.on("can i toggle player ready state", (roomId, playerIndex) => {
            if (!connectedRooms[roomId]["gameStarted"]) {
                  ...
                  io.to(roomId).emit(
                        "you can toggle player ready state",
                        playerIndex
                  );
            }
